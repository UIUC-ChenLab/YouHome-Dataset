...
parser.add_argument('--data_dir',default='/mnt/echo/youhome/toyota/portable',type=str, help='training dir path')
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    if opt.circle:
        criterion_circle = CircleLoss(m=0.25, gamma=64) # gamma = 64 may lead to a better result.
    if opt.triplet:
        miner = miners.MultiSimilarityMiner()
        criterion_triplet = losses.TripletMarginLoss(margin=0.4)
    if opt.lifted:
